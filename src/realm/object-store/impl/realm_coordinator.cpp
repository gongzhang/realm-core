...
void RealmCoordinator::do_get_realm(RealmConfig&& config, std::shared_ptr<Realm>& realm,
                                    ...
                                    std::optional<bool> first_time_open)
{
    ...
    if (subscription_function) {
        ...
        const auto open_for_the_first_time = first_time_open.has_value() ? first_time_open.value() : db_created;
        if (open_for_the_first_time && (current_subscription.version() == 0 || rerun_on_open)) {
